...
// Variables adicionales para controles táctiles
let moviendoIzquierda = false;
let moviendoDerecha = false;

// Modificar la función iniciarJuego para añadir controles táctiles
function iniciarJuego() {
    canvas = document.getElementById('InvasionAlienigena');
    ctx = canvas.getContext('2d');
    ...
    leftBtn.addEventListener('touchstart', () => moviendoIzquierda = true);
    leftBtn.addEventListener('touchend', () => moviendoIzquierda = false);
    leftBtn.addEventListener('mousedown', () => moviendoIzquierda = true);
    leftBtn.addEventListener('mouseup', () => moviendoIzquierda = false);
    
    rightBtn.addEventListener('touchstart', () => moviendoDerecha = true);
    rightBtn.addEventListener('touchend', () => moviendoDerecha = false);
    rightBtn.addEventListener('mousedown', () => moviendoDerecha = true);
    rightBtn.addEventListener('mouseup', () => moviendoDerecha = false);
    
    shootBtn.addEventListener('touchstart', crearBala);
    shootBtn.addEventListener('mousedown', crearBala);
...
// Modificar la función moverNave para incluir controles táctiles
function moverNave() {
    ...
    nave.x += nave.direccionX * nave.velocidad;
    nave.x = Math.max(0, Math.min(canvas.width - nave.ancho, nave.x));
}
